...
* ``render_mode='none'`` (default): no CSVs are produced. Use this for fast headless runs where you do not need UI data, or call :meth:`citylearn.citylearn.CityLearnEnv.export_final_kpis` manually afterwards to obtain the V2 KPI table only.
...
This pattern keeps rendering off (fastest) and emits only the KPI CSV once the run completes. Because timestep histories are not written, the resulting folder can be used with the **KPIs** page of the UI but not with the full time-series explorer. The KPI CSV is generated from :meth:`~citylearn.citylearn.CityLearnEnv.evaluate_v2`, so the export follows the V2 naming tree by default.
...
The KPI-specific tab is documented separately in :doc:`kpis`.
